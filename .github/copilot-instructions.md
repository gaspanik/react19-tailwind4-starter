...
- **Package Manager**: pnpm (default); npm/yarn/bun also work (replace `<pm>` with your package manager)
...
- **pnpm**: Default package manager (see `pnpm-lock.yaml`); npm/yarn/bun also work
...
<pm> run build    # TypeScript check + Vite build
<pm> run check    # Biome lint + format (auto-fix)
```
...
- Run `<pm> run check` before committing. Biome auto-fixes most issues.
...
- **HMR Stopped**: Restart `<pm> run dev`, delete `node_modules/.vite` cache
- **Type Errors**: Pre-check with `<pm> run build` (`tsc -b` → `vite build`)
...
- **Dependencies**: Run `<pm> add <package>`
- **Updates**: Use `<pm> update` to upgrade packages
```
...
- `mise run vite:build` → `<pm> run build` (with confirmation)
- `mise run vite:preview` → `<pm> run preview`

Use mise commands in environments with mise installed, otherwise use `<pm>` directly.

## Common Pitfalls

1. **Don't edit `routeTree.gen.ts`** - it's auto-generated by `@tanstack/router-plugin`
2. **Default package manager is pnpm** - replace `<pm>` with your package manager (npm/yarn/bun also work)
